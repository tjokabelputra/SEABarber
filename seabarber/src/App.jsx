...
function App() {
  ...
  function handleReservation(){
    if(accountDetail.id != null && accountDetail.role == 'Customer'){
      navigate('/reserve');
    }
    else{
      navigate('/login')
  ...
  function handleEdit() {
    ...
    editReview(token, editReviewInfo)
      .then(() => {
        setReview({
          name: accountDetail.username,
      ...
      <nav className='flex flex-row bg-slate-900 justify-between items-center'>
          ...
          <div className='flex-1 flex justify-center'>
            <ul className='my-4 flex flex-row items-center'>
              ...
              <li className='text-2xl text-gray-200 cursor-pointer max-lg:text-xl max-sm:text-xs' onClick={handleDashboard}>{accountDetail.username.slice(0,10)}</li>
              }
            </ul>
          </div>
      ...
      <main className='h-lvh bg-slate-900'>
        ...
        <article className='h-1/2 bg-slate-900 bg-cover bg-center max-sm:h-1/2'>
          ...
          <>
            ...
            <div className='mt-8 flex justify-center max-sm:py-2'>
              <textarea 
                name="comment"
            ...
            <div className='mt-8 flex justify-center max-sm:mt-2'>
              {isEdit === true ? (
                <div className='w-2/5 flex flex-row justify-center max-lg:w-3/5 max-sm:w-4/5'>
                  <button
                  className='w-1/2 py-2 mx-4 text-2xl border-2 rounded-lg text-gray-200 border-red-600 bg-red-600 max-xl:w-3/5 
                  max-lg:w-2/5 max-lg:text-base
                  max-sm:w-1/2 max-sm:text-sm max-sm:mb-2 max-sm:py-1'
                  onClick={handleCancelEdit}>Cancel</button>
                  <button
                  className='w-1/2 py-2 mx-4 text-2xl border-2 rounded-lg text-gray-200 border-blue-600 bg-blue-600 max-xl:w-3/5
                  max-lg:w-2/5 max-lg:text-base 
                  max-sm:w-1/2 max-sm:text-sm max-sm:mb-2 max-sm:py-1'
                ...
                <button 
                className='w-1/4 py-2 text-2xl border-2 rounded-lg text-gray-200 border-green-600 bg-green-600 max-xl:w-3/5 max-lg:w-2/5 
